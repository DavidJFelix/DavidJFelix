...
// through /diag (SvelteKit endpoints under src/routes). The SDKs are imported
// dynamically so they load on demand. Invoked from the root +layout.svelte inside
// onMount, so it runs in the browser only.
//
    ...
    if (sentry.enabled) {
      void import('@sentry/browser').then((Sentry) => {
        Sentry.init({
          ...
          tracesSampleRate: 1,
          // Relay envelopes through our own origin (src/routes/bugs) so blockers
