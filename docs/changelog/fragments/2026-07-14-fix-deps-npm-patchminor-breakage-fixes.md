...
The npm patch/minor renovate batch moved three things that needed hand-fixing before the round could
...
`Cloudflare.Vite` became `Cloudflare.Website.Vite` (alchemy.run.ts). In onvibes.org, bumping the
direct `hono` to 4.12.29 left `@flue/runtime`'s copy resolved at 4.12.27, and the two `Hono` types
no longer unify across that version gap -- `pnpm dedupe hono` collapses the tree back to a single
copy.
