...
test('BaseLayout renders the no-flash theme bootstrap script as the first head script', async () => {
  ...
  // The script is generated by @davidjfelix/theme's createThemeBootstrapScript;
  // the storage key arrives through the serialized options object.
  expect(bootstrapScript).toContain('localStorage.getItem')
  expect(bootstrapScript).toContain('"storageKey":"theme"')
