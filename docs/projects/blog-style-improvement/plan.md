# Blog Style Improvement

## Status

**Active again** (2026-07-01). David ran a full critique of the live site ("criticize everything")
and triaged it into a concrete change list, so style/UX work is moving again. Execution is bucketed:
Claude drives the engineering items; copy and design-taste items are decided with David and reviewed
through per-PR preview URLs (shipped 2026-06-17); a few are backlog or GitHub issues (#308 hero,
#309 off-site SEO). Session detail in `2026-07-01-progress.md`. The #171 discovery punch-list stays
the umbrella for style direction.

## Goal

Apply a human touch to djf.io's visual design and UX. The blog migration produced a functional
baseline; this project polishes it.

## Scope

- Colors
- Spacing
- Layout
- Images
- Usability
- Components

## Workflow

User-directed. David reviews the live site and issues specific changes; Claude executes.

## Related

- Parent umbrella: [djf.io](../djf-io/plan.md)
- Blog migration produced the baseline being polished here (project closed; see the
  [changelog](../../changelog/))
- App location: `apps/djf.io/`
