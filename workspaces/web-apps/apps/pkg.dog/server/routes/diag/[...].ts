...
import {postHogUpstream} from '../../../shared/posthog-proxy'
...
// Runs in the Nitro worker; the client SDK is pointed here in the observability
// plugin (app/plugins/observability.client.ts).
...
// The `[...]` catch-all route matches every method and sub-path under /diag, so
// the proxy is method-agnostic like the upstream.
export default defineEventHandler((event) => proxyToPostHog(toWebRequest(event)))
