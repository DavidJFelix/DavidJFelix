...
import {type ActiveThemeSnapshot, controllerSource, fixedSource} from './theme-source'
...
function makeController(
  initial: ThemeControllerState,
): ThemeController & {set(next: Partial<ThemeControllerState>): void} {
  let state = initial
...
test('controllerSource: maps controller state to a singular active theme snapshot', () => {
...
test('controllerSource: exposes the selected theme names for diffs consumers', () => {
  const controller = makeController({
...
test('controllerSource: notifies subscribers and reflects the new active theme', () => {
  const controller = makeController({
    darkThemeName: 'd',
    lightThemeName: 'l',
    mode: 'light',
...
test('controllerSource: keeps the previous resolved theme until the cold one settles', () => {
...
test('fixedSource: pins a resolved object mode-independently (scheme from its own type)', () => {
...
test('fixedSource: lazily resolves a name and notifies when it settles', async () => {
...
test('fixedSource: a { light, dark } pair resolves the slot matching the provider mode', () => {
  const resolver = makeResolver({})
  const source = fixedSource({light: lightTheme, dark: darkTheme}, {resolver, colorScheme: 'dark'})
  const active = source.getSnapshot()
  expect(active.theme).toBe(darkTheme)
  expect(active.colorScheme).toBe('dark')
})

test('fixedSource: a named { light, dark } pair exposes both names for diffs consumers', () => {
  const resolver = makeResolver({})
