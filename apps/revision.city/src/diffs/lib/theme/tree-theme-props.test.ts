...
import {treeThemeProps} from './tree-theme-props'
...
test('treeThemeProps: without reconcileForegroundFromChrome, returns raw themeToTreeStyles', () => {
  const {style} = treeThemeProps({theme: DIM_FG_THEME, colorScheme: 'dark'})
  expect(style).toEqual(themeToTreeStyles(DIM_FG_THEME))
})

test('treeThemeProps: with reconcileForegroundFromChrome, matches reference fg-upgrade', () => {
...
test('treeThemeProps: returns an empty style when no theme is resolved yet', () => {
