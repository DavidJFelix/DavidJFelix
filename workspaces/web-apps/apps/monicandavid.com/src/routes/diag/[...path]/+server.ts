...
// Runs in the SvelteKit worker; the client SDK is pointed here in
// src/observability/client.
...
// `fallback` handles every method (GET for the SDK bundle/flags, POST for
// capture), so the proxy is method-agnostic like the upstream.
export const fallback: RequestHandler = ({request}) => proxyToPostHog(request)
