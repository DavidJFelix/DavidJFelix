import {COMMIT_HASH_METADATA_PATTERN} from './git-patch-metadata'
...
export function getStreamedPatchMetadata(fileText: string): string | undefined {
  ...
  if (isNullish(diffBoundaryIndex) || diffBoundaryIndex <= 0) {
    return undefined
  }
...
function createGitPatchFileStreamParser(): GitPatchFileStreamParser {
  ...
  function takeAvailableFile(): string | undefined {
    if (isNullish(currentFileBoundaryIndex)) {
    ...
    for (;;) {
      ...
      nextBoundarySearchIndex = isNullish(currentFileBoundaryIndex)
        ? 0
        : currentFileBoundaryIndex + 1
