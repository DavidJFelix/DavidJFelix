...
#[test]
fn test_cast_spell_with_insufficient_health() {
    let mut no_health_wizard = Player {
        health: 1,
        mana: None,
        level: 1,
    };

    let clone = Player { ..no_health_wizard };

    assert_eq!(no_health_wizard.cast_spell(5), 0);
    assert_eq!(no_health_wizard.health, 0);
    assert_eq!(no_health_wizard.mana, clone.mana);
    assert_eq!(no_health_wizard.level, clone.level)
}
