...
It expands on the "Tasks & scripts" rule in [tooling-standard.md](tooling-standard.md): prefer a
`mise` task; if a task is too complex for one, write a script. The first question is always **can
this be a `mise` task calling an existing tool?** If so, do that and stop. Everything below is for
when you genuinely need to write a script.
...
`go run` program. The toolchain, types, and dependencies are already there, contributors already
know the language, and the script can import the project's own code.
...
- **Use Bun's built-ins** instead of shelling out: `Bun.spawn`, `Bun.file`, `fetch`, `Bun.sleep`,
  ...
  error handling, without leaving TypeScript:
...
- **Co-locate tests** (`xxx.test.ts`) and run them with `bun test` -- a script with real logic gets
  a test, same as any other code (`bin/upload-preview.test.ts`).
...
the clearly better fit, not merely possible. Bun is usually enough; Python becomes the obvious
choice when the work is closely tied to its ecosystem -- ML ops, data/ML libraries, a Python-native
tool, or Starlark-adjacent build config. When Python is the right call, use it -- and only ever with
**uv**. `pip` and `poetry` are banned (per [tooling-standard.md](tooling-standard.md)); never invoke
`pip` directly.
...
- **Hard budget: fewer than 10 lines, trivial syntax only** -- straight-line commands, at most a
  ...
  recurring workflow that outgrows scripting entirely can justify a purpose-built utility in Rust or
  Go, fronted by a mise task like everything else.
...
- **Do not commit `.fish` / `.nu` / `.zsh` scripts as project automation.** They are fine as
  personal dotfiles and aliases -- that is what `dotfiles/.config/fish/` is for.
...
- [ ] Python? uv + an explicitly pinned version (PEP 723 or `.python-version`), never
      `pip`/`poetry`?
