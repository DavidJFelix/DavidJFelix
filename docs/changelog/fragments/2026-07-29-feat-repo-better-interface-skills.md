...
The vendored folders are MIT-licensed third-party content: attribution and license text live in
`.agents/skills/NOTICE.md`, `LICENSE.md` lists the exception, and `skills-lock.json` pins the folder
hashes, so the copies are read-only and improvements go upstream. A new
