...
below say which license applies where; each rule links the full license text it relies on.
...
([docs/LICENSE](docs/LICENSE)). The same dedication already applies to [dotfiles/](dotfiles/)
([dotfiles/LICENSE](dotfiles/LICENSE)).
...
([apps/djf.io/src/content/LICENSE](apps/djf.io/src/content/LICENSE)). The djf.io application code
around that writing stays MIT OR Apache-2.0.

## Third-party code

- `apps/revision.city` began as a port of the diffshub application from Pierre Computer Company's
  PierreJS monorepo, which is licensed under Apache-2.0 only. The Pierre-derived portions remain
  Apache-2.0 only -- the MIT option above does not extend to them. See
