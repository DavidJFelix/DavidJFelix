// Reverse-proxy routing for PostHog (US cloud). The on-demand endpoint in
// src/pages/diag/[...path].ts serves these under /diag on this domain so a
// content blocker that drops *.posthog.com can't drop our analytics. PostHog
// splits traffic across two upstreams: the SDK bundle and remote config load
// from the assets host; events, flags, and the rest are ingestion/API traffic.
