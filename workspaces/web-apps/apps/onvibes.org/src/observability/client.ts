...
// through /diag (see src/routes/{bugs,diag}). The SDKs are imported dynamically so
// they stay out of the SSR/worker bundle and load only in the browser. Invoked
// from src/router.tsx, client-side only.
//
    ...
    if (sentry.enabled) {
      void import('@sentry/react').then((Sentry) => {
        Sentry.init({
          ...
          tracesSampleRate: 1,
          // Relay envelopes through our own origin (src/routes/bugs.ts) so
          // blockers that drop *.ingest.sentry.io can't drop real-user errors.
  ...
  if (import.meta.env.VITE_PUBLIC_POSTHOG_KEY) {
    ...
    if (posthog.enabled) {
      void import('posthog-js').then(({default: ph}) => {
        ph.init(posthog.key, {
          // Same-origin reverse proxy (src/routes/diag): events and the lazily
