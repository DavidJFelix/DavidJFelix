...
- **Contract**: `mise run smoke` (declares `depends = ["build"]`) boots the app's _production
  build_ locally and asserts the critical path serves -- each key route returns 200 as a complete
  document whose hashed client assets also serve; an app with a backend also exercises it (f311x
  POSTs the chat endpoint and requires the echo stream). Deterministic and secret-free, so it runs
  on every PR.
...
- Apps with real unit-testable logic gate it with **vitest v8 coverage scoped to the logic** -- not
  ...
  calendar-visualizer and forzamonica.com follow the same recipe. (Under Astro's `getViteConfig`
  the per-file `text` table renders empty -- cosmetic; `text-summary` and threshold enforcement
  work.)
