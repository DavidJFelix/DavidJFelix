    ...
    switch (fileDiff.type) {
      ...
      case 'deleted':
        return Promise.reject(
          new Error(
            `Diffs GitHub file loader cannot hydrate ${fileDiff.type} diffs.`
          )
        );
      case 'change':
      case 'rename-changed':
...
function normalizeLoadedDiffFiles(
  ...
  if (!isRecord(data)) {
    throw new Error(
      'Diffs GitHub file loader returned an invalid response.'
    );
  }
  ...
  if (type === 'rename-pure') {
    if (files.oldFile !== null || files.newFile === null) {
      throw new Error(
        'Diffs GitHub file loader returned an invalid pure rename response.'
      );
    }
  ...
  if (files.oldFile === null || files.newFile === null) {
    throw new Error(
      'Diffs GitHub file loader returned an invalid changed-file response.'
    );
  }
