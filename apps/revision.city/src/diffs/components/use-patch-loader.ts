...
import {
  ...
  streamGitPatchFiles,
} from '@/diffs/lib/stream-git-patch-files';
import type {
  CommentMetadata,
...
export function usePatchLoader({
  ...
  const [treeSource, setTreeSource] = useState<DiffsFileTreeSource | null>(
    null
  );
  const [diffStats, setDiffStats] = useState<DiffsStats | null>(null);
  const [commentFileByItemId, setCommentFileByItemId] =
    useState<DiffsCommentFileByItemId | null>(null);
  ...
  const handleLineLinkChange = useStableCallback(
    (selection: CodeViewLineSelection | null) => {
      const nextHash =
        selection == null ? null : formatDiffsLineHash(selection);
