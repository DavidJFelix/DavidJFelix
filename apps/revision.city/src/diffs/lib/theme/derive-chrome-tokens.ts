/**
 * deriveChromeTokens derives diffs's contrast-heavy "chrome" token set — the
...
// The opinionated app-chrome token set diffsChromeMapping maps onto its CSS
...
function pickReadableMuted(
  ...
  if (isNullish(mutedCandidate) || mutedCandidate === '') return undefined
  ...
  if (isNullish(compositedL) || isNullish(bgL)) {
...
export function deriveChromeTokens(theme: ThemeLike): ChromeTokens | undefined {
  ...
  const separator =
    isNullish(editorBg) || colorUtils.surfacesMatch(editorBg, sidebarBg)
      ? borderOpaque
